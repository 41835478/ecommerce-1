<?php
namespace App\Repositories\admin\cms_cart;

use Session;
use App\Models\CmsCart;
use App\Repositories\admin\cms_cart\CmsCartContract;

class EloquentCmsCartRepository implements CmsCartContract
{
...
$oResults =CmsCart::with('Users','CmsProduct');
    ...
    public function create($data)
    {
        ...
        $result = CmsCart::create($data);

        if ($result) {
            Session::flash('flash_message', 'cms_cart added!');
            return $result->id;
        } else {
            return false;
        }
...
$cms_cart = CmsCart::findOrFail($id);
    ...
    public function destroy($id)
    {
        ...
        $result =  CmsCart::destroy($id);

        if ($result) {
            Session::flash('flash_message', 'cms_cart deleted!');
            return true;
        } else {
            return false;
        }
    ...
    {
$cms_cart = CmsCart::findOrFail($id);
       $result= $cms_cart->update($data->all());
        if ($result) {
            Session::flash('flash_message', 'cms_cart updated!');
            return true;
        } else {
            return false;
        }
