<?php
namespace App\Repositories\admin\cms_compare_list;

use Session;
use App\Models\CmsCompareList;
use App\Repositories\admin\cms_compare_list\CmsCompareListContract;

class EloquentCmsCompareListRepository implements CmsCompareListContract
{
...
$oResults =CmsCompareList::with('Users','CmsProduct');
    ...
    public function create($data)
    {
        ...
        $result = CmsCompareList::create($data);

        if ($result) {
            Session::flash('flash_message', 'cms_compare_list added!');
            return $result->id;
        } else {
            return false;
        }
...
$cms_compare_list = CmsCompareList::findOrFail($id);

        return $cms_compare_list;
    }

    public function destroy($id)
    {

        $result =  CmsCompareList::destroy($id);

        if ($result) {
            Session::flash('flash_message', 'cms_compare_list deleted!');
            return true;
        } else {
            return false;
        }
    ...
    {
$cms_compare_list = CmsCompareList::findOrFail($id);
       $result= $cms_compare_list->update($data->all());
        if ($result) {
            Session::flash('flash_message', 'cms_compare_list updated!');
            return true;
        } else {
            return false;
        }
